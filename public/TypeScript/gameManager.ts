...
import { BattlePanelManager, BattleMove, BattleManager} from "./battleManager.js";
import { fieldMetaData, mapData } from "./config.js";
import { magicKind } from "./Data/specialAttackData.js";
import { musicManager } from "./musicManager.js";


const ereaA_Enemy: string[]=["slime"];
const ereaB_Enemy: string[]=["slime", "golem", "dark-king"];
const Boss_Enemy: string[]=["dark-king"];

let sceneMng: SceneManager;
let panelMng: BattlePanelManager;
let musicMng: musicManager;
let ancientPlayersNameArr: string[] = []; //初期状態
let ancientEnemysNameArr: string[]=[];
let playersNameArr: string[] = []; //バトル中の生きてるプレイヤー
let enemysNameArr: string[] = []; //バトル中の生きてる敵
let players: {[key: string]: Player}={}; //バトル中のプレイヤー情報
let enemys: {[key: string]: Enemy}={}; //バトル中の敵情報
let isBattleFirst: boolean = true; //ゲーム開始後1回目のバトルかどうか

const swap=(array: string[], idx1: number, idx2: number)=>{ //配列のidx1番目とidx2番目を入れ替える
    let str: string="";
    str=array[idx1];
...
const replaceArr=(array: string[], name: string, liveNum: number)=>{
...
async function initProcess(){
    ...
    await sceneMng.drawInit();
    await player.drawPlayerInit();
    fieldProcessInit();
}
...
let player: fieldPlayer = new fieldPlayer(6, 5);
...
let press_key="";
let move=-1;
let walkCount: number=0;
const requestAnimationFrame = window.requestAnimationFrame;
const cancelAnimationFrame = window.cancelAnimationFrame;
let reqFieldProcessID: any;
const fieldProcess=()=>{
    ...
    if(move==-1){
        ...
        let col = move;
        if(col<=8) col=0;
        else if(col<=16) col=1;
        else if(col<=24) col=2;
        else col=1; 
...
//プレイヤーのステータスの色を変える。
const changeStatusColorPlayer=(player: Player)=>{
    if(player.getHp() == 0){
...
//プレーヤーを作成する。
const createPlayerInit=(pObj: {[key: string]: any}, i: number)=>{
...
let commandCount: number = 0; //今のコマンドの数
let isBoss: boolean=false;

let battleMng: BattleManager = new BattleManager();
let livePlayers: number; //生きているプレイヤー数
let liveEnemys: number; //生きている敵の数
let isRunAway=false;
let maxCommandCount: number = 0;


let logCount=0; //ログの回数
let endCommandCount: number = 0; //終了時のコマンド数。このコマンド数で表示を変える。
let nowMove: BattleMove; //今の行動
let isEnemyAppearLog: boolean = true;


const BattleProcess=()=>{
    console.log("BattleProcess");
    
    let isFighting: Boolean=true;
    ...
    for(let i=0;i<enemysNameArr.length;i++){
        let pObj: Enemy = enemyDataObject[enemysNameArr[i]] as Enemy;
    ...
    panelMng.getTargetList().forEach((targetNode)=>{
        targetNode.addEventListener("click",()=>{ //敵の選択ボタンをクリックしたとき
